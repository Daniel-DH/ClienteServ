...
const upload = multer({dest: "C:/Users/danie/Documents/generador/public/images"});
const fs = require('fs');
...
router.post('/edit/:id',upload.single('picture'),async function(req,res){
  try {
    ...
    var editcerebro = await cerebro.findById(req.params.id);
    if(req.file){
      fs.unlink('C:/Users/danie/Desktop/generador/public/images/'+editcerebro.picture, (err) => {
        if (err) throw err;
...
router.post('/delete/:id',async function(req,res){
  try{
    ...
    fs.unlink('C:/Users/danie/Desktop/generador/public/images/'+dcerebro.picture, (err) => {
      if (err) throw err;
      console.log('path/file.txt was deleted');
    });
    dcerebro.remove();
